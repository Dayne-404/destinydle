...
import {
  Weapon,
  WEAPON_COMPARE_FIELDS,
  WEAPON_ORDER,
} from "./config/weaponType";
...
function App() {
  ...
  useEffect(() => {
    ...
    if (gameState > 0) {
      ...
      const finalGameState = setLocalFinishedGameState({
        gameState,
        guessedItems,
        numGuesses: finalGuesses,
      });
  ...
  useEffect(() => {
    const getWeaponItems = async () => {
      try {
        const result: { weapons: Weapon[]; totalCount: number } =
          await getWeapons();
        ...
        <>
          {gameState === 0 && (
            <>
              ...
              />
              <ButtonStack onGiveUp={giveUp} />
            </>
          )}
          <Lives numGuesses={numGuesses} maxGuesses={10} />
          <PreviousWeapon weapon={previousItem} />
          {gameState === 1 && <GameOver dailyItem={dailyItem} />}
          {gameState === 2 && (
            <div ref={targetRef}>
              <GameWon
                dailyItem={dailyItem}
                numTries={11 - (numGuesses || 0)}
              />
            </div>
          )}
          <GuessGrid
